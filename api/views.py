...
def isGunUsed(gun, game):
    ''' Returns if gun is currently in use in a game '''
    players = PlayerInstance.objects.filter(game=game)

    for player in players:
        if gun.id == player.gun.id:
            return True
...
class GameJoin(APIView):
    ...
    def post(self, request, pk, format=None):
        ...
        gun_ = None
        try:
            gun_ = Gun.objects.get(id=data["gun_id"])
            if (isGunUsed(gun_, game)):
                print "Gun in use."
                return Response("Gun frequency in use.")
        except ObjectDoesNotExist:
            gun_ = Gun(id=data["gun_id"],frequency=data["gun_id"])
            gun_.save()
        ...
        instance = PlayerInstance(gun=gun_, player=player, team=team, game=game, num_shots=0, score=0)
        instance.save()
...
def setupPlayer(player, game):
    ...
    try:
        gun_ = Gun.objects.get(pk=player["gun_id"])
    except ObjectDoesNotExist:
        gun_ = Gun(id=player["gun_id"],frequency=player["gun_id"])
        gun_.save()
    ...
    instance = PlayerInstance(gun=gun_, player=player_, team=team, game=game, num_shots=0, score=0)
    instance.save()
    return instance
